#!/bin/sh -e
# Test MPI consistency for Kondo model
...
mkdir -p mpi_consistency_kondo/
cd mpi_consistency_kondo
...
if [ "${result}" = "PASS" ]; then
    echo "MPI consistency test PASSED for Kondo model"
    exit 0
else
    echo "MPI consistency test FAILED for Kondo model"
