...
- **L=10 Hubbard (N=63,504) on 4x A100: FAILS in ELPA** (PBS 859015): Hamiltonian generation and redistribution complete (host MaxRSS 39.6 GB/rank), then ELPA's 1-stage `solve_tridi_single` aborts with `error in cuda_malloc when allocating qmat1_dev` on every rank — the divide-and-conquer work buffer is a single-GPU allocation of order one full N x N matrix (16 N^2 bytes = 64.5 GB > 40 GB), so it does NOT shrink with more GPUs. rc=255, ~5.4 min per attempt.
- **Bracketing (PBS 859016, same config, `ExpecMode 1`, outputmode none)**: SpinGC L=15 (N=32,768) PASS, LapackDiag 84.9 s; canonical Spin L=18 2Sz=0 (N=48,620) PASS, LapackDiag 199.3 s (16 N^2 = 37.8 GB just fits in 40 GB). **Practical A100-40GB ceiling: N ~= 4.9e4, independent of GPU count in this ELPA 1-stage configuration.** Follow-up candidate: ELPA 2-stage on GPU (HPhi currently pins GPU runs to 1-stage) may have a different device-memory profile.
...
- Size-extrapolation validated by these points: CPU memory model 64 N^2 bytes total -> N ~= 2x10^5 feasible on 16 nodes (3.8 TB), diag ~3 h at ~512 ranks; GPU ceiling is device-memory-bound at N ~= 4.9e4 (A100-40GB).
