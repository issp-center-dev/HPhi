...
- 4-GPU / 2-node addition (PBS 858431, F2acc, acc003+acc004, `select=2:ncpus=64:ngpus=2:mpiprocs=2`, `mpirun -n 4 -ppn 2`, `NGPU 2`, `I_MPI_ADJUST_GATHERV=3`): all 4 runs rc=0, "Using ELPA (GPU)". `LapackDiag` at N=16384: **18.97 s** (x2 GPUs 35.51 s -> ~1.87x inter-node scaling; ~102x LAPACK). Small sizes flat/worse (N<=4096: 2.0-3.6 s) — GPU count should scale with matrix size. Script `~/kugui_bench_gpu4.pbs`.
