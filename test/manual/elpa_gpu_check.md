...
- 4-GPU / 2-node addition (PBS 858431, F2acc, acc003+acc004, `select=2:ncpus=64:ngpus=2:mpiprocs=2`, `mpirun -n 4 -ppn 2`, `NGPU 2`, `I_MPI_ADJUST_GATHERV=3`): all 4 runs rc=0, "Using ELPA (GPU)". `LapackDiag` at N=16384: **18.97 s** (x2 GPUs 35.51 s -> ~1.87x inter-node scaling; ~102x LAPACK). Small sizes flat/worse (N<=4096: 2.0-3.6 s) — GPU count should scale with matrix size. Script archived at `test/manual/benchmarks/kugui_bench_gpu4.pbs`.

### 2026-07-20 — kugui, item 18 (L=10 GPU point) + maximum-size validation — CLOSED (boundary found)
- Item 18 was originally scoped for clavius (skipped there for GPU contention); executed on kugui F2acc instead (2 nodes x 2x A100-SXM4-40GB, np=4, `NGPU 2`).
- **L=10 Hubbard (N=63,504) on 4x A100: FAILS in ELPA** (PBS 859015): Hamiltonian generation and redistribution complete (host MaxRSS 39.6 GB/rank), then ELPA's 1-stage `solve_tridi_single` aborts with `error in cuda_malloc when allocating qmat1_dev` on every rank. rc=255, ~5.4 min per attempt. **Corrected analysis (ELPA 2025.06 source, solve_tridi_col_template.F90)**: qmat1_dev is only max_size^2 x 8 B with max_size ~ N/P (~2 GB here) — it was merely the last allocation on an already-full device. The device memory IS distributed: per-GPU peak ~ 40 N^2 / P bytes on the complex 1-stage path (complex A 16N^2/P + complex Q 16N^2/P + real tridiag-stage Q workspace 8N^2/P). At P=4, N=63,504 -> 40.3 GB/GPU (just over 40 GB); N=48,620 -> 23.6 GB (fits). Model boundary N ~= 6.2e4 at 4x A100-40GB, and the ceiling grows as sqrt(P).
- **Bracketing (PBS 859016, same config, `ExpecMode 1`, outputmode none)**: SpinGC L=15 (N=32,768) PASS, LapackDiag 84.9 s; canonical Spin L=18 2Sz=0 (N=48,620) PASS, LapackDiag 199.3 s. **Practical ceiling on kugui (acc queues cap at 2 nodes = 4x A100-40GB): N ~= 6e4 per the 40 N^2/P model above (measured: 48,620 OK / 63,504 NG); more GPUs or 80 GB devices raise it as sqrt(P x M).** Follow-up candidate: ELPA 2-stage on GPU (HPhi currently pins GPU runs to 1-stage) may have a different device-memory profile.
- **Same N=63,504 on CPU (PBS 859017, F4cpu 4 nodes x 32 ranks = 128, `ExpecMode 1`, outputmode none): PASS** — LapackDiag 1375.9 s (~23 min), total wall 1421 s, MaxRSS 2.1 GB/rank (~67 GB/node of 240 GB; consistent with the ~64 N^2-bytes-total peak model, ~4 simultaneous matrix copies). Parallel efficiency vs the 16-rank N=16,384 baseline: ~91%.
- Size-extrapolation validated by these points: CPU memory model 64 N^2 bytes total -> N ~= 2x10^5 feasible on 16 nodes (3.8 TB), diag ~3 h at ~512 ranks; GPU ceiling is device-memory-bound at ~40 N^2/P bytes per GPU (N ~= 6e4 at 4x A100-40GB).

### 2026-07-21 — phase 3c (CSR energy-family trace kernel) validation + benchmark — COMPLETED (break-even analysis)
- Commit under test: phase-3c branch head (49b83d8e). The energy/fluctuation family now runs through a CSR SpMV trace kernel under ExpecMode 2 for every makeHam-reachable model (Hubbard/HubbardGC/tJ/tJGC/Kondo/KondoGC/Spin/SpinGC incl. general spin), replacing the per-state expec_energy_flct fallback.
- **clavius correctness (Release, build_elpa)**: ELPA/expec suite np=2 12/12 + np=3 3/3; end-to-end `fulldiag_expecmode_equiv` np2/np3 PASS — Mode-2 (energy through the kernel) == Mode-0/1 zvo_phys at 1e-8 for all 5 cases + the new tJ orthogonality case (energy kernel ACTIVE while GF fall back) + the InputHam demotion case.
- **clavius Debug (assert-enabled, build_elpa_dbg)**: expec_trace + fulldiag_expecmode_equiv_np2 4/4 PASS — the CSR-invariant asserts (rowptr[0]==0, monotonic, rowptr[n]==nnz; panel row-index guards) hold on real data across all models.
- **Benchmark (clavius, SpinGC chain L=14, N=16384, Solver 3 CPU, np=16, all-states one-+two-body GF, CalcPhys section):** Mode 0 = 474.5 s / Mode 1 = 34.30 s / Mode 2 = 34.05 s. ExpecMode-2 kernel breakdown (rank 0): one-body map 0.000/stream 0.139/output 0.006 s; two-body map 0.010/stream 5.277/output 0.091 s; **energy map 0.086/stream 1.451/output 0.000 s**.
- **Break-even analysis (per plan Task-8 fallback clause):** the energy trace kernel works and is correct, but at N=16384 SpinGC it saves only ~0.25 s (Mode 2 vs Mode 1): the energy family via the kernel costs ~1.5 s and was similarly inexpensive in Mode 1, while the observable time here is dominated by the two-body GF kernel (5.3 s) and the non-kernelized S2. The plan's numeric gate (Mode-2 CalcPhys <= 10.4 s) was calibrated to the kugui 3b value (20.8 s) and is not comparable on clavius (different CPU; Mode-1 baseline is 34.3 s here). The primary phase-3c benefit is therefore **breadth, not raw SpinGC speed**: models whose energy/fluctuation evaluator is expensive, and the tJ/Kondo/general-spin families that the GF kernels do not cover, now get the energy family trace-kernelized (and demonstrably correct) under ExpecMode 2 — previously impossible.
- **kugui NOT run this round:** the develop-merged commit 5bee7b45 broke the Komega Fortran build under Intel MPI (`USE mpi, ONLY : MPI_Allreduce` rejected by Intel Fortran) — filed as issue #291. HPhi cannot currently build with the Intel toolchain on kugui until that is fixed; the phase-3c C code itself builds cleanly on both gcc/OpenMPI (clavius) and, per compile-check, Intel once #291 is resolved.
