#!/bin/sh -e
# Test MPI consistency for HubbardGC model
...
mkdir -p mpi_consistency_hubbardgc/
cd mpi_consistency_hubbardgc
...
if [ "${result}" = "PASS" ]; then
    echo "MPI consistency test PASSED for HubbardGC model"
    exit 0
else
    echo "MPI consistency test FAILED for HubbardGC model"
