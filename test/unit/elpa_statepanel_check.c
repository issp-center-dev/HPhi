...
int main(int argc, char **argv) {
  ...
  /* Failure-propagation phase: rank 0 passes an invalid panel leading
     ...
     ONLY. The synchronized verdict inside RedistBlockCyclicToStatePanel
     must make EVERY rank return -1 without entering the collective
     pzgemr2d_ (a hang here means the verdict Allreduce is broken).
     Requires n > 1 so that lde=1 is genuinely invalid on rank 0. */
  if (n > 1 && size >= 2) {
    long int lde_bad = (rank == 0) ? 1 : lde;
    int rc2 = RedistBlockCyclicToStatePanel(n, Z_ref, descZ, jb + 1, ncols,
                                            lde_bad, panel);
    if (rc2 != -1) {
      fprintf(stderr,
