...
#pragma omp parallel for reduction(+: spn) default(none) firstprivate(i_max, is_up, is1_up, is2_up, isite1, isite2) private(list_1_j, ibit1_up, num1_up, ibit2_up, num2_up, num1_down, num2_down, spn_z, iexchg, off, ibit_tmp) shared(vec)
	...
	if(isite1==isite2){
#pragma omp parallel for reduction(+: spn) default(none) firstprivate(i_max, isite1, X, S1) shared(vec)	
	  for(j=1;j<=i_max;j++){
