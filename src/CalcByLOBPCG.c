...
void zhegv(int itype, char *jobz, char *uplo, int *n, double complex *a, int *lda, double complex *b, int *ldb, double *w, double complex *work, int *lwork, double *rwork, int *info);
...
)
{
  ...
  int ii, jj, jtarget;
  double complex **x, **hx, hsub[9], ovrp[9];
  double eig, dnorm;
  /*
   Variables for zhegv 
  */
  ...
  */
  ...
  c_malloc2(x, 3, X->Check.idim_max + 1);
  c_malloc2(hx, 3, X->Check.idim_max + 1);
...
#pragma omp parallel for default(none) private(i) shared(v0, v1) firstprivate(i_max)
    ...
    for (iproc = 0; iproc < nproc; iproc++) {
      ...
      if (sum_i_max <= iv && iv < sum_i_max + i_max_tmp) {
        ...
        if (myrank == iproc) {
          x[1][iv - sum_i_max + 1] = 1.0;
          if (X->Def.iInitialVecType == 0) {
            x[1][iv - sum_i_max + 1] += 1.0*I;
            x[1][iv - sum_i_max + 1] /= sqrt(2.0);
          }
        }/*if (myrank == iproc)*/
      }/*if (sum_i_max <= iv && iv < sum_i_max + i_max_tmp)*/
...
#pragma omp for
        for (i = 1; i <= i_max; i++)
          x[1][i] = 2.0*(dsfmt_genrand_close_open(&dsfmt) - 0.5) + 2.0*(dsfmt_genrand_close_open(&dsfmt) - 0.5)*I;
      }
      else {
#pragma omp for
        for (i = 1; i <= i_max; i++)
          x[1][i] = 2.0*(dsfmt_genrand_close_open(&dsfmt) - 0.5);
      }

    }/*#pragma omp parallel*/

    dnorm = sqrt(creal(vec_prod(i_max, x[1], x[1])));
#pragma omp parallel for default(none) private(i) shared(x,i_max, dnorm)
    for (i = 1; i <= i_max; i++)  x[1][i] = x[1][i] / dnorm;
  }/*else if(initial_mode==1)*/

   //Eigenvalues by Lanczos method
  TimeKeeper(X, cFileNameTimeKeep, cLanczos_EigenValueStart, "a");

  for (i = 1; i <= i_max; i++)  hx[1][i] = 0.0;
  mltply(X, hx[1], x[1]);
  stp = 1;
  TimeKeeperWithStep(X, cFileNameTimeKeep, cLanczos_EigenValueStep, "a", stp);

  for (i = 1; i <= i_max; i++) x[2][i] = 0.0;
  for (i = 1; i <= i_max; i++) hx[2][i] = 0.0;

  eig = creal(vec_prod(i_max, x[1], hx[1]));

  for (stp = 1; stp <= X->Def.Lanczos_max; stp++) {

    for (i = 1; i <= i_max; i++) x[0][i] = hx[1][i] - eig * x[1][i];
    dnorm = creal(vec_prod(i_max, x[0], x[0]));
    ...
    */
    ...
    if(stp /= 1)
      for (i = 1; i <= i_max; i++) x[0][i] = x[0][i] / dnorm;

    for (i = 1; i <= i_max; i++)  hx[0][i] = 0.0;
    mltply(X, hx[0], x[0]);

    for (ii = 0; ii < 3; ii++) {
      for (jj = 0; jj < 3; jj++) {
        hsub[jj + ii * 3] = vec_prod(i_max, x[jj], hx[ii]);
        ovrp[jj + ii * 3] = vec_prod(i_max, x[jj], x[ii]);
      }
    }
    eig = creal(hsub[2 + 2 * 3]);

    if (stp == 1) {
      zhegv(1, 'V', 'U', 2, hsub, &lda, ovrp, &ldb, eigsub, work, &lwork, rwork, &info);
      jtarget = 0;
    }
    else {
      zhegv(1, 'V', 'U', 3, hsub, &lda, ovrp, &ldb, eigsub, work, &lwork, rwork, &info);
      jtarget = 0;
    }

    eig = 0.5 * (eig + eigsub[jtarget]);

    x[1][i] = x[ii][i] * hsub[ii + jtarget * 3];
    hx[1][i] = hx[ii][i] * hsub[ii + jtarget * 3];
    x[2][i] = hsub[0 + jtarget * 3] * x[0][i] + hsub[2 + jtarget * 3] * x[2][i];
    hx[2][i] = hsub[0 + jtarget * 3] * hx[0][i] + hsub[2 + jtarget * 3] * hx[2][i];

    for (ii = 1; ii < 3; ii++) {
      dnorm = sqrt(creal(vec_prod(i_max, x[ii], x[ii])));
      for (i = 1; i <= i_max; i++) {
        x[ii][i] = x[ii][i] / dnorm;
        hx[ii][i] = hx[ii][i] / dnorm;
      }/*for (i = 1; i <= i_max; i++)*/
    }/*for (ii = 1; ii < 3; ii++)*/
  ...
  double complex *v2) 
{
  ...
  prod = 0.0;
#pragma omp parallel for default(none) shared(v1,v2,ndim,prod) private(idim) reduction(+: prod)
