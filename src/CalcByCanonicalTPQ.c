...
)
{
  ...
  if (X->Bind.Def.iReStart == RESTART_INOUT || X->Bind.Def.iReStart == RESTART_IN) {
    ...
    byte_size = fread(v0, sizeof(complex double), NumAve*(X->Bind.Check.idim_max + 1), fp);
    ...
    StartTimer(3200);
    iret = expec_energy_flct(&(X->Bind), NumAve, v0, v1); //v1 <- v0 and v0 = H*v1
    StopTimer(3200);
    if (iret != 0) return -1;
  ...
  if (X->Bind.Def.iReStart == RESTART_NOT || X->Bind.Def.iReStart == RESTART_OUT || iret == 1) {
    ...
    */
    ...
    for (rand_i = 0; rand_i < rand_max; rand_i++) {
      ...
      fprintf(fp, "%.16lf %.16lf %.16lf %d\n", inv_temp, global_1st_norm[rand_i], global_1st_norm[rand_i], step_i);
      fclose(fp);
    }
    /**@brief
    ...
    */
    X->Bind.Def.istep = 0;
    StartTimer(3300);
    iret = expec_cisajs(&(X->Bind), NumAve, v2, v1);
    StopTimer(3300);
    if (iret != 0) return -1;

    StartTimer(3400);
    iret = expec_cisajscktaltdc(&(X->Bind), NumAve, v2, v1);
    StopTimer(3400);
    if (iret != 0) return -1;
    ...
    for (rand_i = 0; rand_i < rand_max; rand_i++) {
      ...
      fprintf(fp, "%.16lf  %.16lf %.16lf %.16lf %.16lf %d\n", inv_temp, X->Bind.Phys.energy[rand_i], X->Bind.Phys.var[rand_i],
      ...
      fprintf(fp, "%.16lf %.16lf %.16lf %.16lf %.16lf %.16lf %.16lf %d\n", inv_temp, X->Bind.Phys.num[rand_i], X->Bind.Phys.num2[rand_i],
        X->Bind.Phys.doublon[rand_i], X->Bind.Phys.doublon2[rand_i], X->Bind.Phys.Sz[rand_i], X->Bind.Phys.Sz2[rand_i], step_i);
      fclose(fp);
      //
      StopTimer(3600);
    }
  ...
  for (step_i = X->Bind.Def.istep; step_i < X->Bind.Def.Lanczos_max; step_i++) {
    ...
    for (rand_i = 0; rand_i < rand_max; rand_i++) {
      ...
      fprintf(fp, "%.16lf %.16lf %.16lf %d\n", inv_temp, global_norm[rand_i], global_1st_norm[rand_i], step_i);
      fclose(fp);
  ...
  if (X->Bind.Def.iReStart == RESTART_OUT || X->Bind.Def.iReStart == RESTART_INOUT) {
    ...
    fwrite(&v1[0][0], sizeof(complex double), NumAve*(X->Bind.Check.idim_max + 1), fp);
    fclose(fp);
    TimeKeeperWithRandAndStep(&(X->Bind), cFileNameTPQStep, cOutputVecFinish, "a", 0, step_i);
