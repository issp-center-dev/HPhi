  ...
  if (isite1 > X->Def.Nsite) {
    ...
    case Spin:
      if (X->Def.iFlgGeneralSpin == FALSE) {
        ...
        num1 = child_SpinGC_CisAis((unsigned long int) myrank + 1, is1_up, isigma1);
        num2 = child_SpinGC_CisAis((unsigned long int) myrank + 1, is2_up, isigma2);
      }/*if (X->Def.iFlgGeneralSpin == FALSE)*/
...
firstprivate(i_max, dtmp_V, is1_up, isigma1, X, num2) private(j, num1)
          for (j = 1; j <= i_max; j++) {
            num1 = child_Spin_CisAis(j, is1_up, isigma1);
            tmp_v0[j] += dtmp_V * num1 * tmp_v1[j];
          }
        }
      }/* if (X->Def.iFlgGeneralSpin == FALSE)*/
...
private(j, num1, num2)
      for (j = 1; j <= i_max; j++) {
        num1 = child_Spin_CisAis(j, is1_up, isigma1);
        num2 = child_Spin_CisAis(j, is2_up, isigma2);
        tmp_v0[j] += dtmp_V * num1*num2*tmp_v1[j];
      }
    }
    else {
...
private(j, num1, num2)
      for (j = 1; j <= i_max; j++) {
        num1 = child_SpinGC_CisAis(j, is1_up, isigma1);
        num2 = child_SpinGC_CisAis(j, is2_up, isigma2);
        tmp_v0[j] += dtmp_V * num1*num2*tmp_v1[j];
      }
    }
    else {//start:generalspin
