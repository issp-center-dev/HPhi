...
//  if(i_Lz>1){
    ...
    dam_pr1=0.0;
    dam_pr2a=0.0;
    #pragma omp parallel for default(none) reduction(+:dam_pr, dam_pr1,dam_pr2a) private(j) shared(v0, v1)firstprivate(i_max) 
    for(j=1;j<=i_max;j++){
...
//    }else{
//      Lz_i=0
//      X->Phys.e1    = X->Phys.energy;// E^1
//      X->Phys.e2a   = X->Phys.var ;// E^2
//    } 
//
    ...
    solve_2ndPolinomial(X,&alpha_p,&alpha_m,E1,E2a,E2b,E3);
...
//
// Too complicate ?? 
void  solve_2ndPolinomial(struct BindStruct *X,double *alpha_p,double *alpha_m,double E1,double E2a,double E2b,double E3){
  double a,b,c,d;
...
void  Lz(struct BindStruct *X,double alpha,double *Lz_Ene,double *Lz_Var,double E1,double E2,double E3,double E4){
  ...
  *Lz_Ene=(E1+2*alpha*E2+alpha*alpha*E3)/(1+2*alpha*E1+alpha*alpha*E2);
  *Lz_Var=(E2+2*alpha*E3+alpha*alpha*E4)/(1+2*alpha*E1+alpha*alpha*E2);
