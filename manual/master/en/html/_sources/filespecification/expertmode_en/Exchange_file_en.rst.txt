...
.. math::
   ...
   \mathcal H+=\sum_{i,j}J_{ij}^{\rm Ex} (c_ {i \uparrow}^{\dagger}c_{j\uparrow}c_{j \downarrow}^{\dagger}c_{i  \downarrow}+c_ {i \downarrow}^{\dagger}c_{j\downarrow}c_{j \uparrow}^{\dagger}c_{i  \uparrow}),

while for the spin system, they are given as

.. math::

   \mathcal H+=\sum_{i,j}J_{ij}^{\rm Ex} (S_ {i}^{+}S_{j}^{-} + S_ {i}^{-}S_{j}^{+}).

**We note that** :math:`(S_i^{+}S_j^{-} + S_i^{-}S_j^{+})` **in the spin system is written by the operators for electrons as** 
...
*  [string01]
   ...
   **Description :** A keyword for the total number of Exchange
...
*  [int01]
   ...
   **Description :** An integer giving the total number of Exchange
   couplings.
...
*  A program is terminated when the components of the exchange couplings
   are double counted.

*  A program is terminated when [int01] is different
   from the total number of exchange couplings defined in this file.
