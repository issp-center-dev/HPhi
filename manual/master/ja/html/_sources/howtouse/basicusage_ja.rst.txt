...
MPI並列/ハイブリッド並列を用いる場合、プロセス数は以下のように設定してください。
  ...
  * :math:`t`-:math:`J`\ 模型
   ...
   :ref:`Subsec:calcmod` の **CalcMod**\ ファイルで、\ ``CalcModel``\ として\ :math:`t`-:math:`J`\ 模型を選択した場合は、プロセス数が\ :math:`4^n`\ となるように設定してください(fermion Hubbard模型と同じ)。\ :math:`t`-:math:`J`\ 模型の物理的な1サイトは3状態(空・上・下)ですが、内部表現では1サイトあたり4状態を保持するため、プロセス数は4のべき乗(\ **3のべき乗ではありません**\ )です。二重占有した inter-process サイトを持つプロセスは局所次元0となります。
...
MPI並列化の技術的詳細（サイト分類、通信パターン、バッチ通信）については :ref:`Ch:Technical` を参照してください。
