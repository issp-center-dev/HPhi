...
``Flct_tpq.dat`` に出力されます。この集約ファイルには全runが含まれ、
各データ行は ``set step inv_temp N N^2 D D^2 Sz Sz^2`` の順になります。
このモードでは ``Flct_rand??.dat`` は作成されません。
...
-  :math:`[`\ double02\ :math:`]`
   ...
   **説明 :** 全粒子数\ :math:`\sum_{i} \langle n_i \rangle`\ 。
...
-  :math:`[`\ double03\ :math:`]`
   ...
   **説明 :**
   粒子数の2乗の期待値\ :math:`\langle (\sum_{i} n_i)^2 \rangle`\ 。
