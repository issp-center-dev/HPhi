...
-  ``OutputEigenVec``
   ...
   **説明 :** 固有ベクトルの出力の指定を行います。
   ...
   1: 出力あり
   から選択することが出来ます。
   FullDiag計算では、全固有ベクトルが出力されます。MPI実行時も、
   各ファイルには分割されていない1本の固有ベクトルが格納されます。
...
-  ``ExpecMode``
   ...
   **形式 :** int型 (デフォルト値 0)
   ...
   | エネルギー・ゆらぎ系については、可能な理由はちょうど3つで、この
     ...
     このランクで発行する行列要素数で、バッファ容量とゲートはこれに
     合わせて確保・判定されます。重複を合算した後の要素数
     ``rowptr[N]`` はこれより小さくなり得ます) にも適用されます。
     見積もられたCSRサイズ
   ...
   | メモリゲートによるフォールバックが表示されたがトレースカーネルを
     ...
     ベクトルとは異なり、CSRハミルトニアンは各ランクに完全複製される
     ため、MPIランク数を増やしてもCSRは小さくなりません (理由は
     :ref:`Sec:ParallelFullDiag` を参照)。キャップを引き上げるか、表示
     された ``ExpecMode 1`` フォールバック (結果は同一) を受け入れるか
     の2択になります。
