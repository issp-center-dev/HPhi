...
:math:`\mathrm{nnz} \approx T \cdot N` (:math:`T` は1列あたりに寄与
するハミルトニアン項の数) で、固定モデルでは :math:`N` にほぼ比例
します (:math:`T` 自体も系のサイズに対してゆるやかにしか増えません。
例: Hubbard鎖 :math:`L=10` で :math:`T \approx 21`、:math:`L` サイト
横磁場スピン鎖で :math:`T \approx L+1`)。支配的な ``colidx``/``val``
の格納は :math:`\approx 24 \times \mathrm{nnz}` バイト
