...
対応モデルにおけるフォールバックの理由は、この順序で判定される次の
3つです: ハミルトニアンが ``InputHam`` から読み込まれた場合、モデルが
...
* 物理量評価は、対応モデルであれば ``ExpecMode 2``、それ以外は
  ...
  :math:`O(N^2/P)` のメモリ削減のためにプロセス数を増やす場合、
  ``ExpecMode 2`` のエネルギー系CSRハミルトニアンは各ランクに複製
  され :math:`P` では小さくならない点に注意してください (上記の複製に
  関する説明を参照)。通常の格子模型では小さいですが、1ランクあたりの
  メモリの下限を与えるため、:math:`P` が大きい場合や演算子密な入力
  では効いてきます。
