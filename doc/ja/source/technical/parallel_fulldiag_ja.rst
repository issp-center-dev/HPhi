...
* **GPU (Solver 3, A100 40GB)**: ELPA (2025.06) の 1-stage GPU 実装は
  三重対角求解時に **1 GPU あたり** 行列1個分規模のデバイスバッファ
  (:math:`16 N^2` バイト) を確保するため、GPU 枚数を増やしても
  この上限は伸びません。実測では :math:`N=48620` は動作 (対角化
  199秒/4GPU)、:math:`N=63504` は確保失敗で停止しました。
  A100 40GB での実用上限は :math:`N \approx 4.9 \times 10^4` です。
  それ以上の次元は CPU ノードを使用してください。
